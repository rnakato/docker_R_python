for tag in 2024.09 latest
do
#    docker build -t rnakato/r_python:$tag -f Dockerfile.micromamba --target normal .
...
#    docker push     rnakato/r_python:$tag
    docker build -t rnakato/r_python_gpu:$tag -f Dockerfile.micromamba --target gpu .
